...
def generate_adcirc_configuration(
    ...
    if use_nems:
        nems_modeling_system = ensemble_configuration['nems'].nemspy_modeling_system
        run_processors = nems_modeling_system.processors
        run_executable = ensemble_configuration['nems']['executable_path']
    else:
        nems_modeling_system = None
    ...
    if use_nems:
        if tidal_spinup_duration is not None:
            ...
            nems_spinup_modeling_system = ModelingSystem(
                nems_modeling_system.start_time - tidal_spinup_duration,
                nems_modeling_system.start_time,
                nems_modeling_system.interval,
                ocn=deepcopy(nems_modeling_system['OCN']),
                **nems_modeling_system.attributes,
            )
        else:
            nems_spinup_modeling_system = None

        if nems_spinup_modeling_system is not None:
            coldstart_filenames = nems_spinup_modeling_system.write(
                output_directory,
                overwrite=overwrite,
            ...
            coldstart_filenames = nems_modeling_system.write(
                output_directory,
                overwrite=overwrite,
                include_version=True,
                create_atm_namelist_rc=False,
        ...
        if nems_spinup_modeling_system is not None:
            hotstart_filenames = nems_modeling_system.write(
                output_directory,
                overwrite=overwrite,
                include_version=True,
                create_atm_namelist_rc=False,
